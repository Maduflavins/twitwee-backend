...
const getUserDetails = async (req, res) => {
  try {
    ...
    const userDetails = await pool.query(
      'SELECT user_id, username, bio, profile_pic, followers, following, registered_on, follower_id FROM users LEFT JOIN followers ON followers.follower_user = $2 AND followers.following_user = users.user_id  WHERE username = $1',
...
const getFollowers = async (req, res) => {
  ...
  try {
    const stmt1 =
      'SELECT followers.*, username, profile_pic FROM followers INNER JOIN users ON follower_user = users.user_id WHERE following_user = $1 '
...
const getFollowing = async (req, res) => {
  ...
  try {
    const stmt1 =
      'SELECT followers.*, username, profile_pic FROM followers INNER JOIN users ON following_user = users.user_id WHERE follower_user = $1 '
    const stmt2 = ' ORDER BY follower_id DESC LIMIT 100'
    let query = ''
    const values = [userId]
    if (current) {
...
const getFeed = async (req, res) => {
  ...
  try {
    const stmt1 =
      '(SELECT posts.*, username, profile_pic, like_id FROM posts INNER JOIN users ON posts.posted_by = users.user_id LEFT JOIN likes ON likes.post_id = posts.post_id AND likes.user_id = $1 WHERE posts.posted_by IN (SELECT following_user FROM followers WHERE follower_user = $1 UNION SELECT $1 as following_user)'
